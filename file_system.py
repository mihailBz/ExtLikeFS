...
class FileSystem:
    ...
    def get_file_inode_id(self, path: PurePosixPath, return_symlink_inode_id: bool = False) -> int:
        inode_id = 0
        ...
        for child in parents[1:]:
            ...
            if parent_entry.content.get(child.name) is not None:
                ...
                if inode.content.get('file_type') == 'l':
                    data: str = self.read_data(inode.content.get('data_blocks_map')).content
                    symlink_contained_path: PurePosixPath = PurePosixPath(data)
                    if return_symlink_inode_id and path.name == inode.content.get('file_name')[0]:
                        return inode_id
    ...
    def stat(self, path: str) -> Inode:
        inode_id = self.get_file_inode_id(self.resolve_path(path), return_symlink_inode_id=True)
        inode = self.read_inode(inode_id)
        return inode
